...
| **Experience Engine** | `src/core/experience-engine/` | Turns Industry DNA + guidance into a structured Experience Blueprint (cinematic, conversion-focused digital experiences). Horizontal capability that consumes `core/industry-dna`. Interfaces only — see **ADR-012** / PRD-001. |
| **Experience Pipeline** | `src/core/experience-pipeline/` | The staged transformation of Business DNA into an Experience Blueprint — thirteen independently replaceable stages. Consumes `core/industry-dna` and `core/experience-engine`. Interfaces only — see **ADR-013** / PRD-002. |
| **Experience Strategy** | `src/core/experience-strategy/` | The first engine that produces real value: generates a structured Experience Strategy document from a business's details. v0.1 uses mock data (no AI/UI); designed for the Brain to drive later. See **ADR-014**. |
| **Brain Orchestrator** | `src/core/brain-orchestrator/` | The CEO of the platform: coordinates all work (observe · decide · delegate · learn) but never executes it. Departments execute; the Brain delegates. Depends on abstractions (dependency inversion); scales to hundreds of departments unchanged. Interfaces only — see **ADR-015** / PRD-003. |
| **Website Blueprint** | `src/core/website-blueprint/` | Transforms an approved Experience Strategy into a platform-independent Website Blueprint — the master architectural document every future generator consumes. Explainability (confidence + reasoning) on every element. Interfaces only — see **ADR-017** / PRD-005. |
| **Business Spine** | `src/core/business/` | The platform's central entity: the Business record (lifecycle lead → … → account), versioned pipeline artifacts, and the repository abstractions + adapters (in-memory, Supabase). Horizontal — every feature reads it. See **ADR-023**. |
| **Pitch Intelligence** | `src/core/pitch-intelligence/` | Deterministic per-trade sales knowledge (talking points, objections, job values) surfaced in the CRM — the seed of Industry DNA in sales. See **ADR-024**. |
| **Market Intelligence** | `src/core/market-intelligence/` | Per-trade, per-location lead economics behind a `MarketDataProvider` seam: seeded benchmark workbook by default, live providers by env key. Provenance on every number. See **ADR-025**. |
| **Trade Taxonomy** | `src/core/trade-taxonomy/` | The canonical trade + services vocabulary — one id space with market intelligence. Selected, never free-typed, in the main flows. See **ADR-026**. |
| **Pricing** | `src/core/pricing/` | TITAN's sellable-services catalogue (founder-editable placeholders) and the Deal maths (ex-VAT stored, inc-VAT computed). See **ADR-026**. |
...
| **Website Renderer** | `src/features/website-renderer/` | The React/Next.js realisation of the Website Blueprint: hand-crafted primitives composed 1:1 from registry ids (never free-generated layout). Platform-specific by design, so a feature — not core. See **ADR-022** / ADR-021. |
| **Businesses** | `src/features/businesses/` | The pipeline view over the Business Spine: list + per-business Journey. Seed of the command centre. Vertical slice. See **ADR-023**. |
| **CRM** | `src/features/crm/` | The three-level command centre (Pipeline · Build Queue · Accounts) — three views of the Business record, with the founder approval gate. Vertical slice. See **ADR-024**. |
| **Market** | `src/features/market/` | The lead-economics explorer (/market) + the shared estimate card the CRM embeds. Vertical slice. See **ADR-025**. |
